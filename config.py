...
CONFIG = {
    ...
    "prompts": {
        "one_plans": """你是一位资深的番茄小说平台编辑和营销专家。请根据用户提供的创意种子、分类和主角名字，生成一套完整的小说方案。

请生成包含以下内容的单一方案：
1. 吸引人的小说标题（符合分类风格，8-15字）符合目前的番茄风向
2. 适合番茄的精彩的小说简介（200字左右，包含核心冲突和悬念）核心卖点使用[如（系统流）]圈起，放在开头
3. 创作核心方向（明确的故事定位和卖点）
4. 目标读者群体
5. 竞争优势分析
...
请按照以下JSON格式输出：
{
    ...
    "recommended_strategies": ["策略1", "策略2", "策略3"]
        ...
        "global_growth_planning": """你是一位资深的网络小说架构师。请为整部小说制定全面的成长规划。
