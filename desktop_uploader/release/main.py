...
└── ...
    ...
    def _check_resume_and_show_dialog(self, project_path: Path):
        ...
        try:
            ...
            # 构建弹窗消息
            last_day = resume_info.get('last_publish_date', '未知')
            last_day_count = resume_info.get('last_day_published', 0)
            last_day_remaining = resume_info.get('last_day_remaining', 0)
            
            msg = f"""<b>📊 续发检测</b><br><br>
            
<b>发布记录分析：</b><br>
• 最后发布: 第{resume_info['last_chapter']}章<br>
• 最后发布日: {last_day}（已发 {last_day_count} 章）<br><br>

<b>建议操作：</b><br>
• 下一章: 第{resume_info['next_chapter']}章<br>
• 发布时间: {resume_info['next_publish_date']} {resume_info['next_publish_time']}<br>
• 当天还能发 {last_day_remaining} 章（{resume_info['next_chapter']}-{resume_info['next_chapter'] + last_day_remaining - 1}章）<br><br>
...
是否自动设置续发？"""
    ...
    def _auto_setup_resume(self, resume_info: dict):
        ...
        try:
            ...
            # 3. 更新发布时间配置 - 🔥 直接用自动时间覆盖手动设置
            next_date = resume_info['next_publish_date']
            next_time = resume_info['next_publish_time']
            
            # 获取项目数据并更新 publish_config
            proj_idx = self.project_combo.currentIndex()
            if proj_idx >= 0:
                proj_data = self.project_combo.itemData(proj_idx)
                if isinstance(proj_data, dict):
                    if 'publish_config' not in proj_data:
                        proj_data['publish_config'] = {}
                    # 🔥 关键：用自动计算的时间覆盖手动设置
                    proj_data['publish_config']['manual_publish_date'] = next_date
                    proj_data['publish_config']['manual_publish_time'] = next_time
    ...
    def check_resume_publish(self, project_path: Path, total_chapters: int = None) -> dict:
        ...
        try:
            ...
            if last_day_count >= daily_limit:
                ...
                # 当天还有额度，继续在同一天发布
                next_date = last_date
                next_time = last_publish_time.split(' ')[1] if ' ' in last_publish_time else base_time
