...
class MockAPIClient:
    ...
    def call_api(self, system_prompt: str, user_prompt: str, temperature: float = None, purpose: str = "未知", provider: str = None) -> str:
        ...
        elif "提取原著背景资料" in purpose_lower or "extract_original_work_background" in purpose_lower:
            result = self._mock_original_work_background()
        elif "改进原著背景资料" in purpose_lower or "improve_original_work_background" in purpose_lower:
            result = self._mock_improved_original_work_background()
        else:
            # 默认返回小说方案作为通用响应
            result = self._mock_novel_plan()
        import json
...
他不仅要在这个世界活下去。他要成为异界的传奇医者，用现代医学和这个系统的力量，改变整个修罗大陆的医疗格局。这不仅仅是生存，这是使命。"""
    ...
    def _mock_original_work_background(self) -> Dict:
        """模拟原著背景资料提取 - 返回高质量的凡人修仙传背景资料"""
        return {
            "worldview": {
                "世界名称": "人界 - 天南地区、乱星海",
    ...
    def _mock_improved_original_work_background(self) -> Dict:
        """模拟改进后的原著背景资料 - 返回更准确的版本"""
        return {
            "worldview": {
                "世界名称": "人界 - 天南地区、乱星海",
                "时代背景": "修仙时代，实力为尊的丛林法则世界。修仙资源稀缺，修士之间为了功法、丹药、法宝等资源不惜生死相搏",
            ...
            "characters": {
                ...
                "慕沛灵": "落云宗筑基期女修（关键信息：此时为筑基期，非结丹期），容貌绝美，资质出众。因家族逼迫和宗门内元婴长老觊觎而深陷困境，性格外冷内热",
                "梅凝": "拥有极其罕见的'通玉凤髓之体'的特殊体质女子，是绝佳的双修鼎炉，因此被魔道修士觊觎。心地善良，性格坚韧，与兄长梅真人相依为命",
                "温天仁": "乱星海逆星盟高层修士，结丹后期大圆满修士，修炼《六极真魔功》。与韩立有私人恩怨（关键信息：来自乱星海逆星盟高层，非天南魔道六宗成员），心狠手辣，觊觎梅凝的特殊体质"
            },
            "power_system": {
                "体系名称": "凡人流修仙体系",
                "等级划分": "炼气期(共十三层) → 筑基期(分前、中、后三期) → 结丹期(分前、中、后三期及大圆满) → 元婴期(分前、中、后三期) → 化神期",
