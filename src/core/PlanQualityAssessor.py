...
class PlanQualityAssessor:
    ...
    # 评估提示词模板
    SUMMARY_EVAL_PROMPT = """你是一位资深爽文网文编辑，专精于番茄小说市场的质量评估。

请对以下写作计划摘要进行评估，重点关注爽文核心指标：

**爽文专项评估（权重最高）**：
1. 爽点密度 - 是否每2-4章至少有一个小爽点？爽点之间是否有足够的铺垫？
2. 黄金三章质量 - 前3章是否有强钩子、强冲突、首次爽点？严禁慢热！
3. 压抑-爆发配对 - 每个爽点前是否有足够的压抑设计？爽感是否来自压抑的释放？
4. 节奏合规性 - 是否符合番茄快节奏要求？有无水文嫌疑？
5. 收获具象化 - 每次爽点后主角是否有具象收获（财富/地位/能力/人脉）？

**基础质量评估**：
6. 商业吸引力 - 是否有清晰的卖点和差异化
7. 结构完整性 - 爽点单元制结构是否完整
8. 角色一致性 - 角色动机和行为是否自洽
9. 逻辑连贯性 - 剧情发展是否有逻辑漏洞

**评分标准（0-100分，爽文标准）**：
- 90-100分：优秀爽文，可直接使用（爽点密集、节奏快、钩子强）
- 70-89分：良好，有小问题但不影响爽感
- 50-69分：一般，爽点不足或节奏有问题，需要修改
- 0-49分：较差，不适合番茄市场，需要大幅修改

**特别注意**：
- 如果发现"慢热开局"（前3章没有爽点），直接扣30分
- 如果发现"爽点疲劳"（连续爽点无铺垫），扣15分
- 如果发现"抽象收获"（爽点后主角没有具体获得什么），扣10分
...
{{
    "overall_score": 85,
    "readiness": "ready",
    "strengths": ["卖点清晰", "节奏紧凑", "爽点设计到位"],
    "issues": [
        {{
            "category": "pacing",
            "severity": "medium",
            "location": "opening_stage",
            "description": "第2章铺垫略长，建议压缩到1章内完成",
            "suggestion": "加快首次爽点的到来，黄金三章必须每章都有推进"
        }}
    ],
    "summary": "整体质量良好，爽点节奏清晰，建议..."
}}
"""
