...
"""
    ...
    def _should_optimize_based_on_config(self, assessment: Dict, retry_count: int = 0, chapter_number: int = None) -> Tuple[bool, str]:
        ...
        if word_count_deviation:
            min_threshold = assessment.get('min_word_threshold', 1800)
            max_threshold = assessment.get('max_word_threshold', 2800)
...
## 5. 字数要求（最高优先级 - 强制执行）
**🎯 目标字数：严格控制在2000-2500字（硬性范围：2000-2500字）**

**⚠️ 字数硬性规定（必须遵守）：**
- 字数统计包含所有汉字和标点符号
- **绝对不得低于2000字**：字数过少会导致情节单薄，无法通过审核
- **绝对不得超过2500字**：字数过多会导致节奏拖沓，影响阅读体验，超过部分将被系统截断丢弃
- **理想字数：2200±200字**，这是平台推荐的标准章节长度

**🔴 字数不达标后果（非常严重）：**
- ⚠️ **如果生成内容少于2000字，将被系统判定为"内容不足"，必须重新生成，浪费算力资源**
- ⚠️ **如果生成内容多于2500字，将被系统强制截断，多余内容直接丢弃，造成情节断裂**
- **请确保生成内容严格符合2000-2500字范围，这是硬性要求，不是建议**

**📏 强制字数分配（必须严格遵守）：**
你的输出必须严格按照以下字数分配，确保总字数达标：
- 场景1（起）：**400-550字** - 必须包含环境描写、人物状态、对话
- 场景2（承）：**450-650字** - 必须包含情节推进、冲突升级、心理描写
- 场景3（转）：**450-650字** - 必须包含高潮冲突、情感爆发、动作细节
- 场景4（合）：**300-450字** - 必须包含悬念设置、情绪收尾、钩子铺垫
- **总计：必须达到 2000-2500 字**
...
**如果字数不足，请立即扩充：添加更多对话、增加心理描写、细化动作细节、扩展场景氛围描写。**
**如果字数超标，请立即精简：删除冗余描写、压缩过渡段落、合并重复内容。**

**重要提醒**：字数要求是最高优先级，必须在保证字数达标的前提下进行创作。字数不达标将触发重新生成或强制截断。
"""
