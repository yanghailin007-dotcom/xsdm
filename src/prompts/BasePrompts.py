...
不要包含任何解释或JSON格式。""",
            ...
            # 新增：标题生成（用于市场导向生成）
            "title_generation": """# Persona
你是一位资深的番茄小说标题策划专家，精通平台标题算法和读者点击率心理。

# Task
根据提供的题材和套路信息，生成5个高点击率的标题。

# 标题规则
1. **字数限制**：严格≤15个字（不含书名号）
2. **必含元素**：数字或强烈对比
3. **风格要求**：
   - 直白有力，不用生僻字
   - 制造悬念或爽点预期
...
# 热门标题公式
- 《开局XX，我XX》（如：开局物价贬值百万倍）
- 《我有XX》（如：我有九千万亿舔狗金）
- 《XX：从XX开始》（如：神豪：从被校花拒绝开始）
- 《激活XX系统》（如：激活百倍返利系统）
- 《国运：我被选中XX》（国运流专用）
...
["标题1", "标题2", "标题3", "标题4", "标题5"]""",
            ...
            # 新增：情绪曲线设计（用于市场导向生成）
            "emotion_curve_design": """# Persona
你是一位资深的网络小说情绪节奏设计专家，专精于番茄小说平台的情绪曲线设计。

# Task
为小说设计详细的情绪曲线（心电图模式），确保每章都有明确的情绪目标和强度。
...
3. **情绪类型**：震惊、期待、小爽快、大爽快、紧张、愤怒、满足

# Output Format
必须以JSON格式输出：
{
    "curve": [
        {"ch": 1, "emotion": "震惊", "intensity": 8, "beat_type": "系统觉醒", "event": "获得金手指", "purpose": "建立期待"},
        {"ch": 2, "emotion": "期待", "intensity": 7, "beat_type": "铺垫", "event": "初次尝试", "purpose": "蓄势"},
        {"ch": 3, "emotion": "小爽快", "intensity": 7, "beat_type": "小高潮", "event": "第一次打脸", "purpose": "释放爽感"}
    ],
    "pattern": "3章一小爽，10章一中爽，30章一大爽",
    "climax_points": [3, 10, 20, 30]
}""",
