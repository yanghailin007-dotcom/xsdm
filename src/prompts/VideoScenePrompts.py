...
### 起因阶段 (如果适用)
- 开场使用全景或大远景建立环境
- 运镜: 缓慢推镜（push in）或升降镜（crane），营造渐进的紧张感
- 时长: 4-5秒/镜头
- 节奏: 缓慢，信息渐进释放

### 发展阶段 (如果适用)
- 使用中景和近景展示角色互动
- 运镜: 多样化运镜（推拉、摇镜、跟镜），保持视觉新鲜感
- 时长: 3-4秒/镜头
- 节奏: 明快，信息密集

### 高潮阶段 (如果适用)
- 大量使用特写和大特写强调关键细节
- 运镜: 激烈运镜（快速推拉、环绕、手持晃动），配合快速剪辑
- 时长: 2-6秒/镜头（变化幅度大）
- 节奏: 极快，情绪爆发
...
- 镜头编号
- 景别（全景/中景/近景/特写/大特写）
- 运镜方式（推镜/拉镜/摇镜/跟镜/环绕/升降/固定，可组合使用）
- 时长（秒）
- 详细的视觉描述（包含环境、角色、动作、情绪）
- 音频提示（背景音乐、音效、节奏）

**运镜说明**：
- 推镜(Push in): 从远到近，聚焦主体，增强代入感
- 拉镜(Pull out): 从近到远，展现环境，释放情绪
- 摇镜(Pan): 左右或上下移动，展现全景或跟随动作
- 跟镜(Follow): 跟随主体移动，保持动态连贯
- 环绕(Orbit): 围绕主体旋转，展现立体感
- 升降(Crane): 垂直升降，营造气势或视角变化
- 固定(Static): 不移动，稳定拍摄，适合对话或静态场景

生成时请根据叙事需求选择合适的运镜方式，避免过度使用固定镜头。
- 生成提示词（用于AI视频生成的具体描述）
"""
        return prompt
