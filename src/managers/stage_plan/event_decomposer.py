...
**正确的逻辑**:
1. **爽点优先**: 首先规划每个重大事件的"爽点单元"——铺垫（压抑）应该到什么程度、爽点爆发如何设计、收获如何具象化
...
4.  【🔥黄金三章特殊规则 - 留存优先设计】:
    ...
    - **核心目标：留下读者！这是决定小说生死的开局，必须让读者在第3章结尾时产生"必须继续读"的强烈欲望**
    ...
    - **番茄黄金三章设计原则（必须遵守）**：
      * **第1章结尾**：必须有强卡点（悬念/冲突/期待），让读者忍不住点下一章
      * **第2章结尾**：情绪或冲突升级，保持阅读惯性
      * **第3章结尾**：大高潮或大反转，同时埋下新的钩子，让读者产生"这就没了？"的意犹未尽感
      * **爽点前置**：前300字必须有爽点或强吸引力，不能慢热
      * **节奏紧凑**：3章内完成：危机降临→金手指出现→初步破局→获得第一桶金（或 equivalent 的正反馈）
5.  【🔥绝对禁止时间重叠】:
...
【🔥 黄金三章(1-3章)特殊输出格式 - 留存优先设计】
当章节范围为1-3时，composition直接是一个包含单个完整事件的数组，**核心目标是留下读者**：
{{
    "name": "{major_event_skeleton.get('name')}",
    "type": "major_event",
    "role_in_stage_arc": "起 (黄金开局)",
    "main_goal": "留下读者！通过黄金三章设计让读者产生'必须继续读'的强烈欲望",
    "chapter_range": "1-3",
    "composition": [
        {{
            "name": "黄金开局整体事件",
            "type": "medium_event",
            "chapter_range": "1-3",
            "main_goal": "第1章：强卡点留人；第2章：升级冲突；第3章：高潮+新钩子，让读者意犹未尽",
            "plot_outline": [
                "【前300字爽点】主角遭遇危机/获得金手指的强吸引力开场",
                "第1章情节点1...",
                "第1章情节点2...",
                "【第1章结尾卡点】悬念/冲突/期待，必须点下一章",
                "第2章情节点1...",
                "第2章情节点2...",
                "【第2章结尾升级】情绪或冲突升级，保持阅读惯性",
                "第3章情节点1...",
                "第3章情节点2...",
                "【第3章大高潮】大反转或大爽点",
                "【第3章结尾钩子】新的危机/期待，产生'这就没了？'的意犹未尽感"
            ],
            "description": "黄金三章是番茄小说开局特殊设计，核心目标是留存读者。前300字有爽点，每章结尾有卡点，第3章结尾让人意犹未尽",
            "stage_context": {{...}},
            "emotional_derivation": {{...}},
            "alignment_with_stage_arc": {{"position_in_arc": "起", "contribution_to_stage_emotion": "高能开局，极速入戏，让读者停不下来"}},
            "contribution_to_major": "完成开局留存读者的核心目标",
            "special_emotional_events": [...]
        }}
    ],
    "emotional_arc_summary": "黄金开局留存曲线：强吸引→持续沉浸→高潮满足+意犹未尽"
}}

【标准事件(4章及以上)输出格式】
当章节范围大于3章时，composition按爽点单元功能分解为多个中型事件（键名保持兼容：起=铺垫、承=压抑升级、转=爽点爆发、合=收获+新悬念）：
{{
    "name": "{major_event_skeleton.get('name')}",
    "type": "major_event",
...
- **标准事件(4章+)**: composition = {{"起": [...], "承": [...], "转": [...], "合": [...]}} （爽点单元字典：起=铺垫setup、承=压抑升级、转=爽点爆发payoff、合=收获+新悬念transition）

【🔥 黄金三章留存设计铁律 - 必须遵守】
1. **首要目标：留下读者！** 所有设计必须服务于让读者继续阅读
2. **前300字必须有爽点**：不能慢热，必须快速抓住读者
3. **每章结尾必须有卡点**：悬念、冲突升级、新发现，让读者忍不住点下一章
4. **第3章结尾必须让人意犹未尽**：大高潮后埋新钩子，产生"这就完了？"的失落感
5. **3章内完成正反馈循环**：危机→金手指→破局→获得第一桶金（或 equivalent 收获）
6. **节奏紧凑无尿点**：每个情节点都要推动剧情或情绪，不能有水文

注意：
1. **🔥 黄金三章(1-3章)的composition是数组，不是起承转合字典**
2. **🔥 黄金三章是番茄小说特殊开局设计，核心目标是留存读者**
...
2. **独立场景**：每章形成完整的"爽点单元"场景结构（铺垫→发展→小爆发→过渡），不要与其他章重复
...
## 场景构建要求
请为这个章节设计4-6个场景事件，形成完整的"爽点单元"场景结构（铺垫→压抑升级→小爆发/推进→过渡/新期待）。
...
- **sequence**: 场景序号
- **role**: 场景在爽点单元中的角色（起=铺垫/承=压抑升级/转=小爆发或推进/合=过渡或新期待）
- **position**: 场景位置描述
...
"""
    ...
    def _decompose_single_chapter_with_complete_arc(self, medium_event: Dict, major_event: Dict,
        ...
        这个方法专门处理只有1章的中型事件，直接生成一个完整的"爽点单元"场景序列，
        确保单章也能形成完整的爽感节奏（铺垫→小爆发→收获/过渡）。
...
"""
        ...
        prompt = f"""
# 任务：单章中型事件"完整爽点单元场景构建"
{consistency_block}
你需要为一个单章中型事件设计完整的"爽点单元"场景结构。
...
## 【关键要求】完整的爽点单元结构
由于只有1章，**必须**在一个章节内形成完整的"爽点单元"场景结构：
1. **起（铺垫场景）**: 引入本章情境，建立读者期待（如：反派嚣张出场、主角面临挑战）
2. **承（压抑/升级场景）**: 深化冲突，升级紧张感，让读者觉得"主角该出手了"
3. **转（小爆发/推进场景）**: 达到本章的小高潮——可以是主角展现实力、打脸反派、或关键推进。即使不是大爽点，也要让读者感到满足
4. **合（收获/过渡场景）**: 本章冲突的阶段性解决，主角获得小收获，同时为下一章埋下新钩子

## 场景设计要求（爽文专用）
- 总共4-6个场景，每个场景对应爽点单元的某个阶段
- 每个场景都要有明确的情绪强度：low/medium/high
- 情绪强度应该呈现：low → medium → high → medium 的爽感波动曲线
- **关键**：即使是"起"和"承"（铺垫/压抑），也要有"让读者期待后续"的设计，不能是纯水文字
- 场景之间要有流畅的过渡，形成"期待→紧张→小满足→新期待"的循环
- 最后一个场景必须包含吸引读者继续阅读的钩子（新悬念、新冲突、新发现）
