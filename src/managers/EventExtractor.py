...
## 画面构图要求
- **构图方式**：中心构图，物品占据画面70-80%
- **拍摄角度**：正面或45度角展示
- **镜头景别**：特写或大特写

## 国漫修仙法宝风格特征
### 艺术风格
- **画风**：3D国漫动画风格，参考《凡人修仙传》中的法宝渲染
- **质感**：
  - 金属材质要有光泽和反射
  - 灵力材质要有发光效果
  - 古老材质要有岁月痕迹（锈迹、裂纹等）
...
- **细节**：物品纹理、符文、锈迹等细节要清晰
- **比例**：物品比例协调，符合实际比例
- **渲染**：3D渲染质感，要有真实的光影和材质
- **色彩**：饱和度适中，色调偏向冷色或金属色

## 特殊物品类型建议
- **剑/刀类**：锋利感强，寒光闪烁，剑身可有符文或光效
- **法器/宝物**：温润如玉，内敛发光，纹饰精致
- **古老物品**：有岁月痕迹，锈迹、裂纹体现历史感
- **强大法宝**：能量外溢，光效明显，有威压感
...
请根据以上要求，生成一张高质量的**国漫修仙风格法宝剧照**，用于动画制作参考。"""
    ...
    def _generate_human_portrait_prompt(self, name: str, role: str, appearance: str, personality: str, background: str) -> str:
        ...
        # 根据角色定位确定画面风格
        if role == "主角":
            composition = "半身正面像，胸部以上构图"
            expression = "目光坚毅，神情专注，带有修炼者的气质"
            bg_desc = "简洁背景，可有淡淡云雾或光效，突出主角"
        elif role in ["重要配角", "配角"]:
            composition = "全身像或半身像"
            expression = "表情生动，体现角色性格"
            bg_desc = "背景符合角色身份，可添加场景元素"
        else:
            composition = "全身像"
            expression = "自然表情"
            bg_desc = "简洁背景"
        
        prompt = f"""【国漫修仙风格 - 人物角色剧照生成】
...
## 国漫修仙风格特征
### 艺术风格
- **画风**：3D国漫动画风格，参考《凡人修仙传》《完美世界》《斗破苍穹》
- **线条**：清晰流畅，轮廓分明
- **色彩**：鲜明饱和，色调偏暖或偏冷（根据角色气质）
- **光影**：注重立体感，眼神和面部有高光处理
...
请根据以上要求，生成一张高质量的**国漫修仙风格角色剧照**，用于动画制作参考。"""
        return prompt
