...
class EventManager:
    ...
    def enhance_major_events_structure(self, writing_plan: Dict, stage_name: str, stage_range: str) -> Dict:
        ...
        self.logger.info(f"  ✅ 已增强并均匀分配了{len(major_events)}个大事件的结构")
        return writing_plan
    ...
    def validate_stage_event_density(self, writing_plan: Dict, stage_name: str, stage_range: str) -> bool:
        ...
        for event in major_events:
            ...
            if duration < adjusted_requirements["min_major_duration"]:
                duration_ok = False
                self.logger.info(f"  ⚠️ 大事件'{event.get('name')}'持续时间过短：{duration}章，要求至少{adjusted_requirements['min_major_duration']}章")
        
        if not (major_ok and medium_ok and minor_ok and duration_ok):
            self.logger.info(f"  ⚠️ {stage_name}阶段事件密度不符合要求：")
            self.logger.info(f"    重大事件: 实际{actual_major}个, 要求至少{adjusted_requirements['major_events']}个")
            self.logger.info(f"    中型事件: 实际{actual_medium}个, 要求至少{adjusted_requirements['medium_events']}个")
            self.logger.info(f"    小型事件: 实际{actual_minor}个, 要求至少{adjusted_requirements['minor_events']}个")
            self.logger.info(f"    特殊事件: 实际{actual_special}个")
            self.logger.info(f"    大事件最小持续时间: {adjusted_requirements['min_major_duration']}章")
            self.logger.info(f"    章节占用情况: {len(occupied_chapters)}/{stage_length}章被占用")
            return False
        
        # 计算大事件比例
        major_ratio = actual_major / total_events if total_events > 0 else 0
        self.logger.info(f"  ✅ {stage_name}阶段事件密度验证通过")
        self.logger.info(f"  📊 大事件比例：{major_ratio:.1%}）")
        self.logger.info(f"  📊 章节占用：{len(occupied_chapters)}/{stage_length}章")
        return True
    ...
    def supplement_events_with_ai(self, writing_plan: Dict, stage_range: str, creative_seed: str, 
        ...
        if not truly_empty_chapters:
            self.logger.info(f"  ✅ {stage_name}阶段没有真正的空窗期章节，无需情感事件")
            return writing_plan
        
        self.logger.info(f"  🔍 检测到{len(truly_empty_chapters)}个真正空窗期章节: {truly_empty_chapters}")
        ...
        if not selected_chapters:
            self.logger.info(f"  ✅ {stage_name}阶段无需情感事件补充")
            return writing_plan
        
        self.logger.info(f"  🤖 将在{len(selected_chapters)}个关键章节添加情感事件: {selected_chapters}")
        ...
        try:
            ...
            if supplement_result and "emotional_events" in supplement_result:
                ...
                if validated_events:
                    ...
                    self.logger.info(f"  ✅ 成功添加{len(validated_events)}个情感事件")
                    
        except Exception as e:
            self.logger.info(f"  ❌ AI生成情感事件出错: {e}")
    ...
    def _get_truly_empty_chapters(self, writing_plan: Dict, stage_range: str) -> List[int]:
        ...
        self.logger.info(f"  📊 章节占用统计: {stage_range}共{end_chap-start_chap+1}章, 占用{len(occupied_chapters)}章, 空窗{len(empty_chapters)}章")
    ...
    **需要填充的特定章节：{selected_chapters}**
    ...
    def validate_main_thread_continuity(self, writing_plan: Dict, stage_name: str) -> bool:
        ...
        try:
            ...
            if not all_events:
                self.logger.info(f"  ⚠️ 警告：{stage_name}阶段没有任何事件")
            ...
            self.logger.info(f"  📊 {stage_name}阶段连续性检查: 最大间隔{max_gap}章, 允许{max_allowed_gap}章")
            self.logger.info(f"  📊 事件统计: 重大{len(major_events)}个, 中型{len(medium_events)}个, 小型{len(minor_events)}个, 特殊{len(special_events)}个")
            
            if max_gap > max_allowed_gap:
                self.logger.info(f"  ❌ 连续性检查失败：事件间隔过长")
                return False
            ...
            if major_events:
                ...
                if distribution_score < 0.6:
                    self.logger.info(f"  ⚠️ 连续性警告：重大事件分布需要优化 (得分: {distribution_score:.1%})")
            
            self.logger.info(f"  ✅ {stage_name}阶段主线连续性检查通过")
            return True
            
        except Exception as e:
            self.logger.info(f"  ❌ 连续性检查过程出错: {e}")
    ...
    def identify_event_gaps(self, writing_plan: Dict, stage_range: str) -> List[Dict]:
        ...
        self.logger.info(f"  📊 识别到{len(continuous_gaps)}个有上下文的空窗期章节")
    ...
    def export_events_to_json(self, file_path: str = "novel_events.json"):
        ...
        for stage_name, stage_plan in stage_plans.items():
            ...
            if "stage_writing_plan" in stage_plan:
                ...
                self.logger.info(f"  🔍 检测到嵌套结构 stage_writing_plan，使用嵌套数据")
            else:
                # 平面结构：stage_plan -> event_system
                actual_plan = stage_plan
                self.logger.info(f"  🔍 未检测到嵌套结构，使用原始数据")
            
            # 获取事件系统
            event_system = actual_plan.get("event_system", {})
            self.logger.info(f"  🔍 {stage_name}的事件系统键: {list(event_system.keys())}")
            
            # 提取重大事件
            major_events = event_system.get("major_events", [])
            self.logger.info(f"  🔍 {stage_name}的重大事件数量: {len(major_events)}")
            for event in major_events:
                event_data = {
            ...
            self.logger.info(f"  🔍 {stage_name}的中型事件数量: {len(medium_events)}")
            for event in medium_events:
                event_data = {
                    "name": event.get("name", "未命名事件"),
            ...
            self.logger.info(f"  🔍 {stage_name}的小型事件数量: {len(minor_events)}")
            for event in minor_events:
                event_data = {
                    "name": event.get("name", "未命名事件"),
                    "start_chapter": event.get("chapter", event.get("start_chapter", 0)),
        ...
        try:
            ...
            self.logger.info(f"✅ 成功导出 {len(all_events)} 个事件到 {full_path}")
            self.logger.info(f"   📊 事件统计: 重大事件{output_data['events_by_type']['major']}个, "
            ...
            if all_events:
                ...
                for i, event in enumerate(all_events[:5]):
                    ...
                    self.logger.info(f"   {i+1}. 第{event['start_chapter']}章: {event['name']} ({event_type})")
                    
        except Exception as e:
            self.logger.info(f"❌ 导出事件到JSON文件失败: {e}")
        ...
        if timeline_evaluation:
            ...
            self.logger.info(f"✅ 事件线评价已保存: {evaluation_path}")
            self.logger.info(f"   整体评分: {timeline_evaluation.get('overall_score', 'N/A')}/10")
    ...
    def get_available_chapters_for_generation(self, writing_plan: Dict, stage_range: str, 
        ...
        self.logger.info(f"  📊 生成时可用章节检查: 阶段{stage_range}共{end_chap - start_chap + 1}章, 可用{len(available_chapters)}章")
    ...
    def _adjust_density_for_availability(self, requirements: Dict, available_chapters: int, 
        ...
        self.logger.info(f"  📊 密度调整: 可用{available_chapters}/{total_chapters}章, "
...
# -------------------------------------------------------------
    ...
    def evaluate_overall_event_timeline(self) -> Dict:
        ...
        try:
            ...
            if not sorted_events:
                self.logger.info("  ⚠️ 无法进行全书事件线评价：未找到任何已规划的事件。")
            ...
            self.logger.info(f"  ❌ 全书事件线连续性评价过程中发生严重错误: {e}")
    ...
    def strategically_supplement_events(self, writing_plan: Dict, stage_name: str, stage_range: str,
        ...
        """
        self.logger.info(f"  🔄 开始对 {stage_name} 进行统一的事件补充...")

        # 1. 识别真正可用的空窗期章节
        available_chapters = self.get_available_chapters_for_generation(writing_plan, stage_range, "any")
        if not available_chapters:
            self.logger.info("  ✅ 阶段内无可用空窗期，无需补充事件。")
            return writing_plan

        self.logger.info(f"  🔍 找到 {len(available_chapters)} 个可用章节: {available_chapters[:10]}...")
        ...
        if not selected_chapters:
            self.logger.info("  ✅ 根据策略，当前阶段无需补充事件。")
            return writing_plan

        self.logger.info(f"  🎯 策略选择 {len(selected_chapters)} 个关键章节进行填充: {selected_chapters}")
    ...
    def _generate_supplemental_events_with_ai(self, chapters_to_fill: List[int], stage_name: str,
        ...
        **需要填充的特定章节：{chapters_to_fill}**
        ...
        if result and "supplemental_events" in result:
            ...
            self.logger.info(f"  💖 成功生成 {len(events)} 个补充事件。")
            return events
        return []
    ...
    def _integrate_supplemental_events(self, writing_plan: Dict, supplemental_events: List[Dict]) -> Dict:
        ...
        self.logger.info(f"  ✅ 成功整合 {len(supplemental_events)} 个补充事件到计划中。")
        return writing_plan
