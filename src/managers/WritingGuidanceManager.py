...
class WritingGuidanceManager:
    ...
    def __init__(self, stage_plan_manager):
        self.logger = get_logger("WritingGuidanceManager")
    ...
    def get_chapter_writing_context(self, chapter_number: int) -> Dict:
        ...
        if emotional_plan:
            ...
            self.logger.info(f"  💖 成功为第{chapter_number}章生成情绪指导")
            self.logger.info(f"    情感重点: {emotional_guidance.get('current_emotional_focus', '未知')}")
            self.logger.info(f"    情感强度: {emotional_guidance.get('target_intensity', '未知')}")
        else:
            self.logger.info(f"  ⚠️ 第{chapter_number}章的情绪计划为空")
        ...
        if special_guidance.get("has_special_event", False):
            self.logger.info(f"  💝 第{chapter_number}章有情感特殊事件，重点抓住读者兴趣")
