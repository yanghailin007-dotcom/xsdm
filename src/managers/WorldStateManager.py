...
class WorldStateManager:
    ...
    def _is_valid_character_name(self, name: str) -> bool:
        ...
        # 1.3 包含明显非角色的标点符号（保留括号用于角色类型说明）
        if any(char in name for char in "，。！？；：""''""''【】《》""，、；："):
            return False
        
        # 1.4 明确的功能性词汇（绝对不会是角色名的）
        functional_words = {
            # 基础虚词
        ...
        # 完全匹配功能性词汇才拒绝
        if name in functional_words:
            return False
        
        # 1.5 以特定功能词后缀结尾（但这些词独立存在时可能有效，只在作为后缀时检查）
        invalid_suffixes = ["的", "之"]
        for suffix in invalid_suffixes:
            if name.endswith(suffix) and len(name) > 2:  # 长度大于2才检查
                return False
        
        # 1.6 明显是物品/地点的名称模式（非常严格）
        definite_item_patterns = [
            r"^[剑刀枪戟斧钺钩叉鞭锏锤拐棒]{2,}$",  # 纯武器名
            r"^[丹药丸散膏汤剂露酒茶]{2,}$",        # 纯药物名
            r"^[山川湖海江河溪涧泉瀑峰岭崖谷峡]{2,}$",  # 纯地理名
            r"^[金银铜铁玉石珍珠宝石翡翠玛瑙]{2,}$",  # 纯宝物名
        ]
        
        for pattern in definite_item_patterns:
