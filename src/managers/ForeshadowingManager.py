...
class ForeshadowingManager:
    ...
    def __init__(self, novel_generator):
        self.logger = get_logger("ForeshadowingManager")
    ...
    def register_element(self, element_type: str, element_name: str, importance: str, 
        ...
        if target_chapter <= self.current_chapter:
            ...
            self.logger.info(f"  ✅ 注册待发展元素: {element_name} (类型: {element_type}, 目标: 第{target_chapter}章)")
        else:
            self.elements_to_introduce.append(element)
            self.logger.info(f"  ✅ 注册待引入元素: {element_name} (类型: {element_type}, 目标: 第{target_chapter}章)")
    ...
    def clear_stage_elements(self):
        ...
        self.elements_to_develop = [elem for elem in self.elements_to_develop 
                                   if not elem.get('stage_specific', False)]
        self.logger.info("  ✅ 已清除阶段特定的伏笔元素")
    
    def get_context(self, chapter_number: int) -> Dict:
        """获取伏笔上下文 - 增强版本（支持分层上下文压缩）"""
        # 确保所有必要的属性都已初始化
        if not hasattr(self, 'elements_to_introduce'):
            self.elements_to_introduce = []
        if not hasattr(self, 'elements_to_develop'):
        ...
        # 更新当前章节
        self.current_chapter = chapter_number
        self.logger.info(f"  🎭 更新伏笔管理器当前章节: {chapter_number}")
        ...
        for element in self.elements_to_introduce:
            if element.get("target_chapter") == chapter_number:
                ...
                self.logger.info(f"    ✅ 筛选到引入元素: {element.get('name')}")
        ...
        for element in self.elements_to_develop:
            if (element.get("target_chapter") <= chapter_number and
                ...
                self.logger.info(f"    ✅ 筛选到发展元素: {element.get('name')}")
        ...
        # 对历史伏笔元素进行基于章节距离的压缩
        compressed_intro_elements = []
        for element in self.elements_to_introduce:
            element_chapter = element.get("target_chapter", 1)
            compressed_element = context_manager.compress_context(
                element, chapter_number, element_chapter, "plot"
            )
            compressed_intro_elements.append(compressed_element)

        compressed_develop_elements = []
        for element in self.elements_to_develop:
            element_chapter = element.get("target_chapter", 1)
            compressed_element = context_manager.compress_context(
                element, chapter_number, element_chapter, "plot"
            )
            compressed_develop_elements.append(compressed_element)
        ...
        context = {
            ...
            # 🆕 添加压缩后的完整历史记录
            "all_elements_to_introduce": compressed_intro_elements,
            "all_elements_to_develop": compressed_develop_elements
        }

        # 添加上下文大小信息用于调试
        context_size_info = context_manager.get_context_size_info(context)
        if context_size_info.get("is_large", False):
            self.logger.info(f"⚠️ 第{chapter_number}章伏笔上下文较大: {context_size_info['estimated_tokens']} tokens")

        self.logger.info(f"  📊 伏笔上下文构建完成: {len(current_intro_elements)}个引入, {len(current_develop_elements)}个发展")
    ...
    def get_stage_foreshadowing_plan(self, stage_name: str, start_chapter: int, 
        ...
        self.logger.info(f"  ⏰ 生成{stage_name}的伏笔计划...")
        ...
        if foreshadowing_plan:
            ...
            self.logger.info(f"  ✅ {stage_name}伏笔计划生成完成")
            self._print_foreshadowing_plan_summary(foreshadowing_plan)
            return foreshadowing_plan
        else:
            self.logger.info(f"  ⚠️ {stage_name}伏笔计划生成失败，使用默认计划")
    ...
    def _print_foreshadowing_plan_summary(self, foreshadowing_plan: Dict):
        ...
        self.logger.info(f"    ⏰ {stage_name}伏笔计划摘要:")
        
        # 新元素引入
        new_elements = foreshadowing_plan.get("new_elements_introduction", [])
        self.logger.info(f"      新元素引入: {len(new_elements)}个")
        
        # 现有元素发展
        existing_elements = foreshadowing_plan.get("existing_elements_development", [])
        self.logger.info(f"      元素发展: {len(existing_elements)}个")
        ...
        self.logger.info(f"      伏笔强度分布: 轻度{light_chapters}章, 中度{medium_chapters}章, 重度{strong_chapters}章")
