...
## 【强制要求】情绪强度分布 (Emotional Intensity)
你**必须**为每个场景设置不同的情绪强度，以营造张弛有度的节奏。情绪强度分为三个等级：
- **low** (低强度): 平和、内敛、细腻，用于开场、铺垫、过渡场景
- **medium** (中等强度): 稳健、平衡、自然，用于发展、推进场景
- **high** (高强度): 激昂、强烈、冲击，用于高潮、爆发、转折场景

**具体要求**：
1. **开场场景** (opening): 通常使用 **low** 或 **medium** 强度
2. **发展场景** (development1/2): 逐步递增，从 **medium** 到 **high**
3. **高潮场景** (climax): **必须**使用 **high** 强度
4. **回落场景** (falling): **high** 或 **medium** 强度
5. **结尾场景** (ending): 根据需要使用 **low** 或 **medium** 强度

每个场景的 JSON 中必须包含 `"emotional_intensity": "low/medium/high"` 字段。
"""
        try:
            result = self.generator.api_client.generate_content_with_retry(
...
- 开场场景 (medium): 处理上一章遗留问题
- 发展场景 (medium): 向最终解决推进
- 高潮场景 (high): 中型事件的核心解决时刻
- 回落场景 (medium): 处理后果和影响
- 结尾场景 (low): 中型事件收尾，衔接后续内容
### 跨章连贯性要求：
- 确保章节间场景的平滑过渡
- 保持情绪发展的连贯性
