...
def test_local_server():
    ...
    try:
        ...
        if result.get('success'):
            ...
            print(f'   最小金额: ¥{result["data"]["min_amount"]}')
            print(f'   最大金额: ¥{result["data"]["max_amount"]}')
        else:
    ...
    try:
        ...
        if result.get('success'):
            ...
            print(f'   赠送: {result["data"]["bonus"]}点')
            print(f'   总计: {result["data"]["total_points"]}点')
        else:
    ...
    try:
        ...
        if result.get('success'):
            ...
            print(f'   请求方法: {result["data"]["method"]}')
            print(f'   商户订单号: {result["data"]["pay_params"]["out_trade_no"]}')
            
            # 构建完整支付URL
            pay_params = result['data']['pay_params']
            query = '&'.join([f'{k}={v}' for k, v in pay_params.items()])
            full_url = f'{result["data"]["pay_url"]}?{query}'
            print(f'\n   完整支付URL（可复制到浏览器）:')
    ...
    try:
        ...
        if result.get('success'):
            ...
            print(f'   创建时间: {time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(result["data"]["created_at"]))}')
        else:
    ...
    try:
        ...
        if result.get('success'):
            ...
            if result['data']['status'] == 'completed':
                print('   ✅ 订单已支付完成')
            if result['data'].get('paid_at'):
                print(f'   支付时间: {time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(result["data"]["paid_at"]))}')
    except Exception as e:
        print(f'   ❌ 请求失败: {e}')
    ...
    try:
        ...
        if result.get('success'):
            ...
            print(f'   累计获得: {result["data"]["total_earned"]}点')
            print(f'   累计消费: {result["data"]["total_spent"]}点')
        else:
            print(f'   ❌ 获取余额失败')
    except Exception as e:
        print(f'   ❌ 请求失败: {e}')
...
def main():
    ...
    # 测试3: 本地服务器流程
    print('\n\n' + '='*60)
    print('注意: 开始测试本地服务器流程（请确保服务器已启动）')
    print('='*60)
    time.sleep(1)
    test_local_server()
