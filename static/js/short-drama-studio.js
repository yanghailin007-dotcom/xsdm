...
class ShortDramaStudio {
     ...
     * 渲染视频生成步骤
     */
    async renderVideoStep() {
        const container = document.getElementById('videoContent');
        if (!container) return;
        
        // 🔥 首先尝试从服务器重新加载项目数据（确保有最新的shots）
        try {
            if (this.currentProject?.id) {
                const response = await fetch(`/api/short-drama/projects/${this.currentProject.id}`);
                const data = await response.json();
                if (data.success && data.project) {
                    // 🔥 智能合并：优先保留本地的 shots 和 storyBeats
                    const localShots = this.currentProject.shots;
                    const localStoryBeats = this.currentProject.storyBeats;
                    this.currentProject = { ...data.project };
                    // 如果本地有数据而服务器没有，保留本地数据
                    if (localShots && localShots.length > 0) {
                        this.currentProject.shots = localShots;
                    }
                    if (localStoryBeats) {
                        this.currentProject.storyBeats = localStoryBeats;
                    }
                    console.log('🎬 [视频步骤] 已重新加载项目数据');
                    console.log('🎬 [视频步骤] shots数量:', this.currentProject.shots?.length || 0);
                }
            }
        } catch (error) {
            console.warn('🎬 [视频步骤] 重新加载项目数据失败:', error);
        }
        
        // 如果没有项目数据，显示空状态
        // 🔥 支持两种数据结构：episodes（旧）或 shots（新）
        const hasShots = this.currentProject?.shots && this.currentProject.shots.length > 0;
        const hasEpisodes = this.currentProject?.episodes && this.currentProject.episodes.length > 0;
        
        console.log('🎬 [视频步骤] 检查数据:', { hasShots, hasEpisodes, project: this.currentProject?.title });
        
        if (!this.currentProject || (!hasShots && !hasEpisodes)) {
            container.innerHTML = `
                <div class="empty-state">
                    <p>🎬</p>
                    <p>暂无视频生成任务</p>
    ...
    async loadVideoShots() {
        ...
        // 获取所有镜头 - 支持两种数据结构
        // 1. 从 episodes 数组获取（原有结构）
        // 2. 从 project.shots 获取（新结构）
        let allShots = [];
        
        // 优先从 project.shots 获取（新分镜生成方式）
        if (this.currentProject?.shots && this.currentProject.shots.length > 0) {
            allShots = this.currentProject.shots;
            console.log('📊 [视频步骤] 从 project.shots 加载:', allShots.length, '个镜头');
        } else {
            // 兼容旧结构：从 episodes 获取
            const episodes = this.currentProject?.episodes || [];
            episodes.forEach(ep => {
            ...
            console.log('📊 [视频步骤] 从 episodes 加载:', allShots.length, '个镜头');
        }
