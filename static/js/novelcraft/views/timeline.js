...
export function openEditorFromTimeline(index) {
  ...
  const md = project.files?.['detailed-outline.md']?.content || '';
  // 只要是包含"第X章"标题的节点，都尝试作为章节处理
  const isChapterNode = node.type === 'chapter' || (node.title && /第[\d一二三四五六七八九十百千]+章/.test(node.title));
  if (isChapterNode) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (start !== -1 && i > start && lines[i].match(/^#{1,3}\s*第[\d一二三四五六七八九十百千]+章/)) {
        end = i;
        break;
      }
    }
