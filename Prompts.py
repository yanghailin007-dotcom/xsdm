...
```json
{
    ...
    "language_features": [
        "语言节奏与句式特点（如：多用短句、口语化表达）。",
        "词汇风格与感官冲击力（如：用词直白、多用动词和拟声词）。",
        "其他关键语言技巧（如：系统提示的格式化表达方式）。"
    ],
    "narrative_pace": {
        "opening": "开篇（前3章）的核心任务、节奏要求和要达成的读者感受。",
        "middle": "中期（上架后）的核心循环和升级路线设计，以维持读者兴趣。",
        "late": "后期冲突升级后的节奏控制与爽点转变策略。"
    },
    "dialogue_style": "主要角色（主角、关键配角、反派）的对话特点、功能和风格。",
    "description_focus": {
        "primary": "必须投入最多笔墨详写的核心卖点（如：金手指效果、关键战斗）。",
        "secondary": "对爽点有重要影响的次要元素（如：关键道具、角色反应）。",
        "simplified": "为保证节奏需要简化或一笔带过的内容（如：无关场景、次要人物内心戏）。"
    },
