...
class MajorEventManager:
    ...
    def get_current_major_event(self, chapter_number: int) -> Optional[Dict]:
        """获取当前章节所属的重大事件"""
        writing_plan = self.generator.novel_data.get("writing_plan", {})
        major_events = writing_plan.get("major_events", [])
    ...
    def is_major_event_chapter(self, chapter_number: int) -> bool:
        """判断是否为重大事件章节"""
        return self.get_current_major_event(chapter_number) is not None
    ...
    def get_event_progress(self, chapter_number: int, event: Dict) -> Dict:
        ...
        return {
            ...
            "stage": self._get_event_stage(progress_ratio)
        }
    
    def _get_event_stage(self, progress_ratio: float) -> str:
        """根据进度获取事件阶段"""
...
**事件名称**: {event['name']}
**事件类型**: {event['type']}
**当前进度**: {progress['current_progress']}/{progress['total_chapters']}章 ({progress['stage']})
**事件目标**: {', '.join(event['goals'])}

## 本阶段写作重点:
{self._get_stage_focus(progress['stage'], event)}

## 关键要求:
- 保持事件内部的连贯性和节奏感
- 突出{progress['stage']}的特点和张力
- 为后续阶段做好铺垫
- 确保角色发展和情节推进同步

## 事件专属元素:
{event.get('special_elements', '')}
"""
        return event_prompt
    
    def _get_stage_focus(self, stage: str, event: Dict) -> str:
        """获取各阶段的写作重点"""
        focus_map = {
            "开局阶段": f"建立事件基础，引入核心冲突，展示{event['name']}的规模和挑战",
            "发展阶段": "深化矛盾，角色成长，推进事件核心目标，增加复杂性",
            "高潮阶段": "冲突激化，关键转折，情感爆发，决定性时刻",
            "收尾阶段": "解决主要冲突，展示后果，为后续影响做铺垫",
            "结局阶段": "完整收尾，总结收获，衔接主线剧情"
        }
...
class ForeshadowingManager:
    ...
    def __init__(self, novel_generator):
        ...
        self.introduced_elements = set()  # 已正式出场的元素
            ...
            for name, data in elements.items():
                ...
                if current_chapter < intro_chapter and intro_chapter - current_chapter <= 10:
                    ...
                    if intro_chapter - current_chapter <= 3:
                        intensity = "strong"  # 强烈铺垫
                    elif intro_chapter - current_chapter <= 6:
                        intensity = "medium"  # 中等铺垫
                    else:
                        intensity = "light"   # 轻度铺垫
        ...
        for element_type, elements in opportunities.items():
            if elements:
                ...
                for element in elements:
                    ...
                    prompt_parts.append(f"- **{element['name']}** (计划第{element['planned_intro_chapter']}章出场):")
                    prompt_parts.append(f"  距离出场还有{element['chapters_until_intro']}章，{element['intensity']}铺垫")
                    prompt_parts.append(f"  建议铺垫方式: {methods}")
...
class NovelGenerator:
    def __init__(self, config):
        ...
        self.quality_assessor = QualityAssessor(self.api_client, config)  # 先初始化 quality_assessor
        self.content_generator = ContentGenerator(self.api_client, config, self.quality_assessor)  # 然后传入 content_generator
        ...
        self.subplot_ratio = {
            "main": 0.7,        # 主线比例
            "emotional": 0.15,  # 感情线比例
            "foreshadowing": 0.15  # 伏笔线比例
        }

        # 暗线剧情跟踪
        self.novel_data["subplot_tracking"] = {
            "foreshadowing_lines": [],  # 伏笔线列表
            "emotional_lines": [],      # 感情线列表  
            "subplot_chapters": {       # 专门推进暗线的章节
                "foreshadowing": [],    # 伏笔推进章节
                "emotional": []         # 感情推进章节
            },
            "ratio": self.subplot_ratio  # 记录配比设置
        }
        ...
        # 正确初始化管理器
        self.major_event_manager = MajorEventManager(self)
    ...
    def _initialize_subplot_plan(self):
        ...
        for i in range(5, total_chapters, 7):
            if i <= total_chapters:
                foreshadowing_chapters.append(i)
        
        # 感情线推进章节 (每4-6章安排一次感情推进)  
        emotional_chapters = []
        for i in range(4, total_chapters, 5):
            if i <= total_chapters:
                emotional_chapters.append(i)
        ...
        if user_choice[0] is not None:
            try:
                ...
                if 1 <= choice <= 3:
                    ...
                    print(f"✓ 自动选择方案 1: 《{selected_plan['title']}》")
                    return selected_plan
            except ValueError:
                print("\n⚠️  输入无效，自动选择方案1")
                selected_plan = plans[0]
    ...
    # 在 novel_generator.py 中添加安全的参数获取方法
    ...
    def prepare_batch_chapter_params(self, start_chapter: int, end_chapter: int) -> List[Dict]:
        ...
        for chapter_num in range(start_chapter, end_chapter + 1):
            ...
            # 重大事件特殊处理
            if hasattr(self, 'major_event_manager'):
                major_event = self.major_event_manager.get_current_major_event(chapter_num)
                if major_event:
                    progress = self.major_event_manager.get_event_progress(chapter_num, major_event)
                    event_prompt = self.major_event_manager.generate_major_event_prompt(chapter_num, major_event, progress)
                    params["major_event_info"] = event_prompt
                    params["is_major_event"] = True
                    params["event_stage"] = progress['stage']
                    # 为重大事件章节添加专属要求
                    params["event_specific_requirements"] = self._get_event_specific_requirements(progress['stage'])
                else:
                    params["major_event_info"] = ""
                    params["is_major_event"] = False
                    params["event_specific_requirements"] = ""
            else:
                # 如果没有重大事件管理器，设置默认值
                params["major_event_info"] = ""
                params["is_major_event"] = False
    ...
    def _prepare_basic_chapter_params(self, chapter_num: int) -> Dict:
        """准备基础章节参数"""
        # 原有的参数准备逻辑
        plot_direction = self.get_plot_direction_for_chapter(chapter_num, self.novel_data["current_progress"]["total_chapters"])
        chapter_focus = self.get_chapter_focus(chapter_num, self.novel_data["current_progress"]["total_chapters"])
    ...
    def _quick_optimize_chapter(self, chapter_data: Dict, assessment: Dict) -> Optional[Dict]:
        ...
        optimization_params = {
            ...
            "priority_fix_1": priority_issues[0] if len(priority_issues) > 0 else "提升质量",
    ...
    # 在 novel_generator.py 中修改 generate_and_optimize_chapter 方法
    def generate_and_optimize_chapter(self, chapter_number: int, total_chapters: int) -> Optional[Dict]:
        ...
        # 确保参数完整
        if not self._validate_chapter_params(chapter_params):
            print(f"❌ 第{chapter_number}章参数验证失败")
            return None        
        ...
        # 生成前情提要
        previous_summary = self._get_cached_previous_summary(chapter_number)
        
        # 获取章节衔接提示
        connection_note = self.get_chapter_connection_note(chapter_number)
        ...
        # 确保章节标题唯一性
        chapter_data = self.ensure_chapter_title_uniqueness(chapter_data, chapter_number, plot_direction["plot_direction"])
        ...
        if optimize_needed:
            ...
            # 修复：调用 quality_assessor 的方法而不是不存在的方法
    ...
    def _assess_foundation_completeness(self, content: Dict, content_type: str) -> str:
        ...
        for field in fields:
            if not self._get_nested_value(content, field):
                missing_fields.append(field)
        
        if missing_fields:
            return f"基本完整 (缺失: {', '.join(missing_fields)})"
        else:
            return "完整"
    ...
    def generate_subplot_chapters(self, start_chapter: int, end_chapter: int):
        ...
        for chapter_num in range(start_chapter, end_chapter + 1):
            if (self.is_subplot_chapter(chapter_num, "foreshadowing") or 
                ...
                subplot_chapters.append(chapter_num)
    ...
    def full_auto_generation(self, creative_seed: str, total_chapters: int = None):
        """全自动生成完整小说 - 修复 total_chapters 未设置问题"""
        print("🚀 开始全自动小说生成...")
        ...
        if not self.novel_data["is_resuming"]:
            ...
            self.novel_data["character_design"] = self.content_generator.generate_character_design(
                ...
                self.novel_data.get("custom_main_character_name")  # 传递自定义主角名字
            )
        ...
        for batch_start in range(start_chapter, total_chapters + 1, actual_chapters_per_batch):
            ...
            if batch_end < total_chapters:  # 不是最后一批次才延迟
                print(f"等待{batch_delay}秒后继续下一批次...")
                time.sleep(batch_delay)
    ...
    def load_project_data(self, filename: str) -> bool:
        ...
        # 恢复基本数据
        self.novel_data["novel_title"] = data["novel_info"]["title"]
        self.novel_data["novel_synopsis"] = data["novel_info"]["synopsis"]
        self.novel_data["creative_seed"] = data["novel_info"]["creative_seed"]
        self.novel_data["selected_plan"] = data["novel_info"]["selected_plan"]
        self.novel_data["category"] = data["novel_info"].get("category", "未分类")
    ...
    # 在 novel_generator.py 中添加伏笔追踪方法：
    ...
    def validate_main_character_usage(self):
        ...
        if generated_chapters:
            chapters_with_name = 0
            for chap_num, chapter_data in generated_chapters.items():
                content = chapter_data.get("content", "")
                if custom_name in content:
                    chapters_with_name += 1
    ...
    def print_generation_summary(self):
        ...
        if len(self.novel_data["generated_chapters"]) > 1:
            good_connections = 0
            for i in range(2, len(self.novel_data["generated_chapters"]) + 1):
                if i in self.novel_data["generated_chapters"]:
                    connection = self.novel_data["generated_chapters"][i].get("connection_to_previous", "")
                    if connection and "自然承接" in connection:
                        good_connections += 1
    ...
    """
        ...
        try:
            ...
            if new_title and len(new_title.strip()) > 0:
                ...
                if is_unique and len(new_title) >= 4:  # 确保标题有足够长度
                    return new_title
                else:
                    # 递归重试
    ...
    def _generate_deterministic_title(self, original_title: str, chapter_number: int) -> str:
        ...
        # 最后的手段：添加章节号确保唯一性
        final_title = f"第{chapter_number}章 {base_title}"
        return final_title
    ...
    def _initialize_subplot_plan_with_ratio(self):
        """根据配比初始化暗线推进计划 - 修复 total_chapters 访问问题"""
        # 确保 total_chapters 已经设置
        ...
        # 验证配比
        total_ratio = self.subplot_ratio.get("main", 0.7) + self.subplot_ratio.get("emotional", 0.15) + self.subplot_ratio.get("foreshadowing", 0.15)
    ...
    def _generate_evenly_distributed_chapters(self, count: int, total: int, line_type: str) -> List[int]:
        ...
        # 根据暗线类型确定起始位置
        if line_type == "emotional":
            # 感情线从第5章左右开始，避免过早
            start_chapter = max(5, 1)
        elif line_type == "foreshadowing":
            # 伏笔线可以从第3章开始
            start_chapter = max(3, 1)
        else:
            start_chapter = 1
    ...
    def validate_and_adjust_ratio(self):
        """验证并调整配比，确保合理性"""
        total = self.subplot_ratio.get("main", 0) + self.subplot_ratio.get("emotional", 0) + self.subplot_ratio.get("foreshadowing", 0)
    ...
    def get_chapter_focus(self, chapter_number: int, total_chapters: int) -> str:
        """确定本章的重点内容 - 使用配比版本"""
        return self.get_chapter_focus_with_ratio(chapter_number, total_chapters)  
      
    def get_chapter_focus_with_ratio(self, chapter_number: int, total_chapters: int) -> str:
        """根据配比确定本章的重点内容"""
        # 首先检查是否为重大事件章节
        major_event = self.major_event_manager.get_current_major_event(chapter_number)
        if major_event:
            progress = self.major_event_manager.get_event_progress(chapter_number, major_event)
            return f"重大事件[{major_event['name']}] - {progress['stage']} - {progress['current_progress']}/{progress['total_chapters']}章"


        is_emotional = chapter_number in self.novel_data["subplot_tracking"]["subplot_chapters"]["emotional"]
        is_foreshadowing = chapter_number in self.novel_data["subplot_tracking"]["subplot_chapters"]["foreshadowing"]
        
        if is_emotional and is_foreshadowing:
            return "本章同时推进感情线和伏笔线，注意平衡发展"
        elif is_emotional:
            return "本章重点推进感情线，发展角色关系和情感冲突"
        elif is_foreshadowing:
            return "本章重点推进伏笔线，埋设新伏笔或回收旧伏笔"
        else:
            plot_direction = self.get_plot_direction_for_chapter(chapter_number, total_chapters)
            return plot_direction["main_plot_progress"]
