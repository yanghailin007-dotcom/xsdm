...
def generate_storyboard():
    ...
    try:
        ...
        ai_storyboards = {}
        all_ai_shots = []
        storyboard_by_unit = {}  # 🔥 按重大事件组织分镜头数据（前端期望的格式）
      ...
      "shot_sequence": [
        {
          ...
          "veo_prompt": "画面场景描述（静态）：人物状态/表情/姿态/环境/光线/质感。例如：林启，满脸汗水与血污，双眼布满血丝，牙关紧咬，双手紧握废铁片对准背部，废铁片反射幽冷金属光泽，昏暗废弃坑道环境。",
          "dialogue": {
          ...
          "visual": {
            "description": "动作序列描述（动态）：发生了什么/镜头运动/情节推进。例如：林启猛地挥下废铁片，画面快速剪辑，展现他咬牙切开自己背部坏死皮肉的瞬间，鲜血与碎肉飞溅，露出白森森的脊柱。"
          },
          "duration_seconds": 8
        }
      ]
...
【veo_prompt编写规范】（画面场景 - 静态描述）
1. 直接使用角色名，系统会自动匹配对应的参考图
2. 描述某一帧的**静态画面**：人物状态、表情、姿态、环境、光线、质感
3. 格式：「{角色名}，{外貌表情}，{姿态状态}，{场景环境}，{光影氛围}」
4. 用**姿态词**表达空间关系，避免人物重叠：
   - 单人姿态：站立/端坐/跪地/倒地/蜷缩/前倾
   - 交互姿态：悬空/被扼住/扑向/后退/对峙
   - 位置关系：面前/身后/身旁/脚下
5. shot_type已定义构图范围，不要重复说"特写镜头"这种
6. 示例：
   - 特写：聚焦机械手扼住咽喉，金属手指嵌入，青筋暴起，冷色调
   - 中景：林启站立，右臂机械手扼住尸狗咽喉使其悬空，废墟背景虚化
   - 全景：林启巍然站立，尸狗扑来被截停，完整废弃工厂环境
7. ❌ 不要写动作过程（如"挥下"、"切开"），这些放在visual.description中

【visual.description编写规范】（动作序列 - 动态描述）
1. 描述**发生了什么**：动作过程、情节推进
2. 用箭头连接动作序列：A → B → C
3. 示例：「机械手瞬间伸出 → 卡住尸狗咽喉 → 尸狗四肢挣扎无力 → 主角冷漠面对」
4. 可以包含镜头语言：快速剪辑/慢镜头/推近/拉远
5. 与veo_prompt配合：veo_prompt描述静态画面，visual描述动态过程
...
**⚠️ 对话场景设计建议**
- 当情节点涉及多个角色时，优先设计对话场景展示角色互动
- 对话是推动剧情和展示角色性格的关键，尽量让角色之间有交流
- 对话要简洁有力，符合短剧快节奏特点
