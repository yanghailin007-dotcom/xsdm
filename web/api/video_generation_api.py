      ...
      "shot_sequence": [
        {
          ...
          "veo_prompt": "画面场景描述（静态）：人物状态/表情/环境/光线/构图。例如：林启，满脸汗水与血污，双眼布满血丝，牙关紧咬，双手紧握废铁片对准背部，废铁片反射幽冷金属光泽，昏暗废弃坑道环境。",
          "dialogue": {
...
2. 描述某一帧的**静态画面**：人物状态、表情、姿态、环境、光线、构图
3. 格式：「{角色名}，{外貌表情}，{姿态状态}，{场景环境}，{光影氛围}」
4. 多角色场景：「{角色A}在左侧{状态}，{角色B}在右侧{状态}，{场景描述}」
5. ❌ 不要写动作过程（如"挥下"、"切开"），这些放在visual.description中

【visual.description编写规范】（动作序列 - 动态描述）
1. 描述**发生了什么**：动作过程、镜头运动、情节推进
2. 可以包含时间顺序：先...然后...最后...
3. 可以包含镜头语言：快速剪辑、慢镜头、推近、拉远等
4. 例如：「角色猛地挥下武器，画面快速剪辑，展现击中瞬间的血肉飞溅，镜头拉远展示全场」
5. 与veo_prompt配合：veo_prompt描述动作前的静态画面，visual描述动作过程
