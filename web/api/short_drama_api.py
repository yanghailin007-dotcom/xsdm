...
1. **角色 (Characters)**
   - 提取所有出现的角色，包括：
     * 人类角色（主角、配角、路人）
     * 怪兽、神兽、妖怪等 fantasy 生物（如：狰（五尾一角兽）、饕餮、麒麟等）
     * 动物、宠物等有生命的生物
   - 保留完整的角色名称，包括括号内的描述（如："狰（五尾一角兽）"）
   - 记录外貌特征、服装、表情、特殊标记（如尾巴数量、角、花纹等）
   - 对于怪兽/神兽，必须详细描述：身体结构、毛色/鳞色、眼睛、特殊器官
...
# ============================================
# 获取 frame_sequences.json
# ============================================

@short_drama_api.route('/frame-sequences', methods=['GET'])
def get_frame_sequences():
    """
    获取项目的 frame_sequences.json 文件
    ...
    try:
        ...
        # 构建文件路径
        frame_seq_path = VIDEO_PROJECTS_DIR / project_title / f'{episode}集_创意导入' / 'frame_sequences.json'
        
        logger.info(f'[FrameSequences] 请求文件: {frame_seq_path}')
        
        if not frame_seq_path.exists():
            logger.warning(f'[FrameSequences] 文件不存在: {frame_seq_path}')
            return jsonify({
                'success': False,
                'error': '文件不存在'
            }), 404
        ...
        # 读取并返回 JSON
        with open(frame_seq_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        
        return jsonify(data)
        
    except Exception as e:
        logger.error(f'[FrameSequences] 获取失败: {e}')
        import traceback
        logger.error(traceback.format_exc())
        return jsonify({
            'success': False,
