...
【绝对铁律 — 每章必须以此格式开头】
每一章正文必须以以下格式开始，缺少此标记会导致解析失败：
### 第{actual_start}章 [抓眼球的悬念/冲突型标题]

然后写正文。下一章继续：
### 第{actual_start + 1}章 [抓眼球的悬念/冲突型标题]

以此类推，直到第{actual_end}章。章节之间用一个空行分隔，不要插入分隔线或说明文字。
...
严格按照细纲写作，每章2000字左右。禁止输出任何说明文字、总结、分析、字数统计、写作思路、本章完、待续等标记。"""
