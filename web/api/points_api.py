...
def login_required_api(f):
    """API登录验证装饰器 - 支持 JWT Token 和 Session 两种方式"""
    @wraps(f)
    ...
    def decorated_function(*args, **kwargs):
        ...
        if token and token != '__session__':
            ...
            if error:
                return jsonify({'success': False, 'error': error['message'], 'code': error['code']}), 401
            is_admin = payload.get('is_admin', False)
            g.current_user = {'user_id': payload['user_id'], 'username': payload['username'], 'is_admin': is_admin}
        # Session 验证
...
def get_balance():
    """获取当前点数余额"""
    from flask import g
    user_id = g.current_user.get('user_id') if g.get('current_user') else session.get('user_id')
    if not user_id:
