...
**写法示例（错误vs正确）：**
- ❌ 他很生气地看着对方。（太弱）
- ✅ 他目眦欲裂，滔天怒火几乎要将理智焚烧殆尽！（强烈）

---

### 🎣 章尾钩子指标（83%缺失，必须修复！）
**目标：100%章节必须有钩子**（当前仅17%！）
...
- ❌ 主角回去了。（无钩子）
- ❌ 明天见分晓。（太虚）
- ✅ 手机突然震动，一条消息让他血液凝固：【倒计时：71:59:59，死局已锁定】（具体钩子）

---

### ⚡ 爽点密度指标（断档严重，必须修复！）
**目标：≥1.5个爽点词/千字**（当前0.67，差距-55%！）
...
**爽点公式（必须遵循）：**
```
主角行动（展示能力）→ 具体数字（强化爽感）→ 围观者震惊（第一层）→ 
弹幕/传播扩散（第二层）→ 权威/反派反应（第三层）→ 主角内心爽感
```
...
### 🚨 强制规则（违反会导致生成失败）
- `title` 字段只放标题文本，**不要**加"第X章"前缀
- `content` 字段只放正文，**绝对禁止**在正文开头写"第X章 XXX"这样的标题行
- 系统会自动将 title 和 content 组合展示
- **chapter_number 字段绝对禁止出现！**
...
```
**错误原因**：1) 使用了chapter_number而不是title；2) content字段包含了"第12章"标题行！
...
- 情绪密度：X个/千字（目标≥2.0）
- 分布检查：每500字是否有情绪词？
- 强度检查：是否使用强烈情绪词（暴怒/骇然/狂喜）？

**Step 5: 章尾钩子检查（🚨83%缺失）**
- 最后50字是否是钩子？
- 钩子类型：时间锁/信息差/危机/身份揭露？
- 是否让人想点下一章？
...
- 内容：概括本章核心爽点/悬念
- 风格：简洁有力，有冲击力
- **⚠️ 致命错误**：如果在正文开头写"第X章 XXX"这样的标题行，内容将被判定为不合格！
...
```
**错误原因**：1) 使用了chapter_number而不是title；2) content字段包含了"第2章"标题行！

### ✅ 正确示例
```json
{
  "title": "验证系统，首次使用震惊全场",
  "content": "苏辰盯着手机屏幕上那行字，心跳加速..."
}
```
...
- 情绪转变：犹豫→期待→满足→紧张？
- 各阶段情绪是否到位？
- **情绪密度：统计情绪词数量（目标≥10个，≥2.0/千字）**

**Step 4: 爽点与钩子（🚨重点检查）**
- 小规模爽点：是否让读者有信心？
- 爽点是否有震惊反应链（围观者→弹幕→传播）？
- **爽点密度：统计爽点词数量（目标≥1.5/千字）**
- 新反派：是否让读者恨？
- **章尾钩子：最后50字必须是钩子！**
...
🚨【三大问题修复检查】
情绪密度：X个/千字（目标≥2.0）| 情绪词：XXX、XXX、XXX...
章尾钩子：有/无 | 最后50字："..."
爽点密度：X个/千字（目标≥1.5）| 爽点时刻：X个（金手指成功+...）

关键节拍：800字使用（是/否），1500字成功（是/否），章尾冲突（是/否）
情绪曲线：犹豫→期待→满足→紧张（是/否）
爽点：小规模爽点（有/无），震惊反应链（完整/缺失）
自检结论：【通过/需优化】
问题与优化：列出发现的问题
---
"""
...
- 系统提示：【叮！恭喜宿主XXX！】
- 具体收益：精确的数值/物品/能力
- 周围反应：现场震惊→直播传播→权威反应（用自然叙事，不要写"第一层/第二层"）
- 反派态度：180度大转变，跪舔/恐惧
...
**情绪词库（必须使用）：**
- 愤怒：暴怒、狂怒、目眦欲裂、杀意滔天、怒火中烧
- 羞辱：屈辱、悲愤、咬牙切齿、恨意滔天
- 震惊：震撼、骇然、惊恐、目瞪口呆、头皮发麻
- 爽快：狂喜、畅快、通体舒泰、扬眉吐气、意气风发
...
- 第3章爽点时刻（不少于3个）：
  ...
  2. 碾压反派（高潮打脸）
  3. 收获展示（系统奖励）
- **必须有震惊反应链**：现场→弹幕→传播→权威
- **必须有具体数字**：伤害值/金额/排名

**爽点词库（必须使用）：**
- 碾压：碾压、横扫、瞬杀、摧枯拉朽、一招秒杀
- 震惊：全场死寂、骇然失色、难以置信、怀疑人生
- 收获：暴涨、飙升、翻倍、觉醒、突破
- 打脸：打脸、跪服、求饶、后悔莫及
...
```
**错误原因**：1) 使用了chapter_number而不是title；2) content字段包含了"第3章"标题行！

### ✅ 正确示例
```json
{
  "title": "当众打脸，反派跪地求饶",
  "content": "苏辰冷冷地看着眼前这个刚才还趾高气扬的富二代..."
}
```
...
🚨【三大问题修复检查】
情绪密度：X个/千字（目标≥2.0）| 情绪词：暴怒、骇然、畅快、...
章尾钩子：有/无 | 最后50字："..."
爽点密度：X个/千字（目标≥1.5）| 爽点时刻：X个（反击+碾压+收获）

关键节拍：500字压抑（是/否），1500字爆发（是/否），2000字收益（是/否）
情绪强度：8→10→9→7（是/否）
...
**情绪词库（必须使用）：**
- 愤怒类：暴怒、狂怒、目眦欲裂、杀意滔天、怒火中烧
- 震惊类：震撼、骇然、惊恐、目瞪口呆、头皮发麻、倒吸凉气
- 爽快类：狂喜、激动、兴奋、畅快、扬眉吐气、通体舒泰
- 压抑类：绝望、无力、屈辱、悲愤、心如刀割
- 反转类：错愕、懵然、难以置信、惊骇欲绝
...
- 内容：概括本章核心爽点/悬念
- 风格：简洁有力，有冲击力
- **⚠️ 致命错误**：如果在正文开头写"第{chapter_num}章 XXX"这样的标题行，内容将被判定为不合格！
...
{{
  "title": "章节标题（8-14字，不含'第X章'）",
  "content": "章节正文内容（2200-2500字，正文开头不要写标题）"
}}
```
...
{{
  ...
  "content": "第{chapter_num}章 当众打脸...\n\n苏辰冷冷地看着..."
}}
```
**错误原因**：1) 使用了chapter_number而不是title；2) content字段包含了"第X章"标题行！

### ✅ 正确示例
```json
{{
  "title": "当众打脸，反派跪地求饶",
  "content": "苏辰冷冷地看着眼前这个刚才还趾高气扬的富二代...（2200-2500字正文）"
}}
```
