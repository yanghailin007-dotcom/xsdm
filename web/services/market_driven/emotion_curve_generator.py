...
- 不要套用固定模板"""
        ...
        for ch in range(1, total_chapters + 1):
            if ch not in existing_chs:
                ...
                if prev and next_b:
                    ...
                    next_intensity = next_b.intensity
                    
                    # 计算与前一章的差值，确保至少差1-2
                    gap = next_intensity - prev_intensity
                    chapters_between = next_b.ch - prev.ch - 1
                    
                    if chapters_between > 0:
                        # 在当前位置插值，但加入波浪变化
                        progress = (ch - prev.ch) / (next_b.ch - prev.ch)
                        base_intensity = prev_intensity + gap * progress
                        
                        # 加入波浪：奇数位升，偶数位降
                        wave = 1.5 if ch % 2 == 1 else -1.5
                        intensity = int(base_intensity + wave)
                        intensity = max(4, min(10, intensity))  # 限制范围
                    else:
                        intensity = (prev_intensity + next_intensity) // 2
                    ...
                    # 奇数位降，偶数位升，形成波浪
                    if ch % 2 == 1:
                        intensity = max(5, prev.intensity - 2)
                    else:
                        intensity = min(9, prev.intensity + 1)
                    emotion = "紧张" if intensity < 6 else "期待"
                else:
                    # 前面没有章节（理论上不应该发生）
                    intensity = 6
                    emotion = "期待"
    ...
    def _get_default_curve(self, total_chapters: int, genre: str) -> List[EmotionBeat]:
        ...
        # 前30章使用固定模板（优化版：确保情绪起伏≥2）
        template = [
            (1, "压抑", 9, "钩子", "被羞辱，绝望开局"),
            (2, "反转", 7, "转折", "系统觉醒，点燃希望"),  # 强度7，与第1章差2
            (3, "爽快", 9, "爽点", "第一次大打脸"),  # 强度9，与第2章差2
            (4, "震惊", 7, "钩子", "震惊余波，新目标"),  # 强度7，与第3章差2
            (5, "爽快", 9, "爽点", "大打脸"),  # 强度9，波浪起伏
            (6, "期待", 7, "铺垫", "新场景探索"),  # 强度7，波浪
            (7, "紧张", 9, "钩子", "神秘人物出现"),  # 强度9，波浪
            (8, "爽快", 7, "爽点", "中级打脸"),  # 强度7，波浪
            (9, "震惊", 9, "震惊", "身份曝光"),  # 强度9，波浪
            (10, "期待", 7, "钩子", "大场面预告"),  # 强度7，波浪
            (11, "紧张", 9, "铺垫", "大场面开启"),  # 强度9，波浪
            (12, "爽快", 7, "推进", "一路装逼"),  # 强度7，波浪
            (13, "爽快", 9, "爽点", "连续打脸"),  # 强度9，波浪
            (14, "震惊", 7, "震惊", "全场震惊余波"),  # 强度7，波浪
            (15, "大爽快", 9, "高潮", "阶段性大高潮"),  # 强度9，高潮
            (16, "满足", 7, "收获", "享受成果"),  # 强度7，过渡
            (17, "期待", 9, "钩子", "更大舞台开启"),  # 强度9，新阶段
            (18, "爽快", 7, "爽点", "新圈子打脸"),  # 强度7，波浪
            (19, "震惊", 9, "震惊", "新身份曝光"),  # 强度9，波浪
            (20, "期待", 7, "钩子", "中期BOSS出现"),  # 强度7，波浪
            (21, "压抑", 9, "铺垫", "遇到更强对手"),  # 强度9，BOSS压迫
            (22, "紧张", 7, "推进", "准备反击"),  # 强度7，蓄力
            (23, "爽快", 9, "爽点", "打脸"),  # 强度9，反击
            (24, "震惊", 7, "震惊", "实力展示余波"),  # 强度7，波浪
            (25, "期待", 9, "钩子", "更大冲突预告"),  # 强度9，波浪
            (26, "紧张", 7, "铺垫", "对手反击"),  # 强度7，波浪
            (27, "爽快", 9, "爽点", "化解并反击"),  # 强度9，波浪
            (28, "爽快", 7, "爽点", "连续打脸"),  # 强度7，波浪
            (29, "震惊", 9, "震惊", "震惊全场"),  # 强度9，波浪
            (30, "大爽快", 9, "高潮", "中期大高潮"),  # 强度9，高潮
        ]
