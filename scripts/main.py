...
def main():
    ...
    if not any(CONFIG["api_keys"].values()):
        self.logger.info("❌❌❌❌ 请先设置API密钥")
        return
    
    self.logger.info("🤖🤖🤖🤖 番茄小说智能生成器（优化版本）")
    self.logger.info("="*50)
    self.logger.info("特点: 优化生成速度，智能跳过优化，缓存机制")
    self.logger.info("功能: 全自动小说生成，质量评估，AI痕迹检测")
    self.logger.info("="*50)
    ...
    try:
        ...
        if not creative_seed:
            ...
            self.logger.info(f"使用默认创意: {creative_seed}")
        ...
        if existing_projects:
            ...
            for i, project in enumerate(existing_projects, 1):
                self.logger.info(f"  {i}. {project['title']} ({project['completed_chapters']}/{project['total_chapters']}章) - {project['stage']}")
            ...
            if choice == "1" and existing_projects:
                ...
                if generator.load_project_data(selected_project["filename"]):
                    ...
                    self.logger.info(f"\n📊📊 章节完整性报告:")
                    self.logger.info(f"  预期章节: 1-{integrity_report['expected_total']} (共{integrity_report['expected_total']}章)")
                    self.logger.info(f"  实际文件: {integrity_report['actual_files']}章")
                    self.logger.info(f"  内存数据: {integrity_report['memory_chapters']}章")
                    self.logger.info(f"  完成度: {integrity_report['completion_rate']}%")
                    
                    if integrity_report['missing_chapters']:
                        self.logger.info(f"  ❗❗ 缺失章节: {integrity_report['missing_chapters']}")
                    
                    if integrity_report['memory_but_no_file']:
                        self.logger.info(f"  💾💾 需重新保存: {integrity_report['memory_but_no_file']}")
                    try:
            ...
            if view_content and generator.novel_data["generated_chapters"]:
                ...
                if 1 in generator.novel_data["generated_chapters"] and 2 in generator.novel_data["generated_chapters"]:
                    ...
                    if len(chap1_content) > 300:
                        self.logger.info(chap1_content[-300:])
                    else:
                        self.logger.info(chap1_content)
                    
                    self.logger.info(f"\n第二章开头预览:")
                    chap2_content = chap2['content']
                    if len(chap2_content) > 300:
                        self.logger.info(chap2_content[:300])
                    else:
                        self.logger.info(chap2_content)
                    
                    connection = chap2.get("connection_to_previous", "")
                    if connection:
                        self.logger.info(f"\n衔接说明: {connection}")
        else:
            self.logger.info("❌❌❌❌ 小说生成过程中出现错误")
            
    except KeyboardInterrupt:
        self.logger.info("\n\n收到中断信号，正在保存进度...")
        generator.project_manager.save_project_progress(generator.novel_data)
        self.logger.info("进度已保存，可以安全退出。")
        return
...
def start_new_project(generator, creative_seed):
    ...
    try:
        ...
        self.logger.info(f"使用默认章节数: {total_chapters}")
    
    # 大规模生成警告
    if total_chapters > 100:
        self.logger.info(f"\n⚠️  警告: 即将生成{total_chapters}章小说，这可能需要很长时间")
        self.logger.info("建议: 可以先生成较少章节测试效果")
        confirm = input("确定要继续吗？(y/n): 默认y").lower()
        if confirm == 'n':
            self.logger.info("已取消生成")
            return False
    
    self.logger.info(f"\n开始创建新项目并生成{total_chapters}章小说...")
    self.logger.info("优化版本特点:")
    self.logger.info("✓ 智能API调用优化，减少等待时间")
    self.logger.info("✓ 快速质量评估，智能跳过优化")
    self.logger.info("✓ 缓存机制，避免重复计算")
    self.logger.info("✓ AI痕迹检测和消除")
    self.logger.info("首先将基于番茄小说流量趋势为您提供三套方案")
    self.logger.info("请选择您喜欢的方案后，系统将自动开始创作")
